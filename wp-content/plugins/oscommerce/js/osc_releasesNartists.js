...
var artReltabNames = [ {
	tab : allArtReleases
} ];// , { tab : "Singles" }, { tab : "Albums" }, { tab : "Appears On" }, { tab : "Videos" } ];
...
(function($) { // this defines a scope so we cannot simply split this up into multiple files
	$(function() {
		...
		// our local database where we put everything
		var prodmap = {};
		var manumap = {};
		var relemap = {};
		var prntmap = {};
		...
		var format; // the tab of release format
		// var artRelTab; // the tab of the artist releases
		var artistSet; // the tab of the arist sets
		...
		function toggleProduct(curTabCtx, prodId, format, seltorName) {
			...
			if (lastProductId == prodId) {
				...
				$('div#release-format-tabs.wp-tabs  > div.ui-tabs').children('div.ui-tabs-panel:visible').find('h3').css(
						'display', 'none');
				return;
			} else {
...
//			console.assert(newcart.entries == undefined);
			...
			console.log('fixed %d cart entries: total items %d Sum %s', newcart.entries.length, newcart.total,
					newcart.totalPrice);
			console.dir(newcart.entries);
			return newcart;
		}
...
//			console.log('getProductMaster(%d): %o', prod_id, thisProd);
		...
		function getParameterFromUrl(name, defval, url) {
			...
			if (parm == "null" || url == undefined)
				...
				parm = decodeURI((RegExp(name + '=' + '(.+?)(&|$)').exec(location.search) || [ , null ])[1]).toLowerCase();
			if (parm == "null")
				parm = defval;
...
//				console.log('found previous name value pair %s', oldNameValue);
		...
		function showArtistGridHeader() {
			var tabHeaderJQ = $('div#artist-set-tabs.wp-tabs  > div.ui-tabs').children('div.ui-tabs-panel:visible')
					.find('h3');
			console.log('found %d tabHeader for %o', tabHeaderJQ.length, tabHeaderJQ);
			tabHeaderJQ.html(function(i, html) {
